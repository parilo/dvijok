...
public class FadeIn implements GFX {
	...
	public void start() {
		...
		final double step = endOpacity/stepsCount;
		t = new Timer(){
			@Override
			public void run() {
				opacity += step;
				if( opacity >= 1 ){
					t.cancel();
					widget.getElement().getStyle().setOpacity(endOpacity);
	...
	@Override
	public void init() {
		widget.getElement().getStyle().setOpacity(startOpacity);
	}
